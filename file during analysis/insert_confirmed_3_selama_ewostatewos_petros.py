...
REPORT_PATH = os.path.join(ANALYSIS_DIR, "insert_confirmed_3_report.txt")
EXPANDED_CSV_PATH = os.path.join(ANALYSIS_DIR, "confirmed_3_mappings.csv")

# User-verified dates:
# GZ-MEL-038  Abune Selama Kesate Birhan (Frumentius):
#             consecration Tahsas 18, departure Hamle 26 (NOT the monthly-day-26 cycle)
# GZ-MEL-135  Abune Ewostatewos: main tazkar Meskerem 18 (birth Hamle 21 not used;
#             Meskerem 18 shared with Absadi - this book is Ewostatewos)
# GZ-MEL-158  Peter & Paul joint martyrdom: Hamle 5 (corrected from wrong Sene 5)
BOOKS = [
    ("GZ-MEL-038", "ታኅሣሥ 18, ሐምሌ 26"),
    ("GZ-MEL-135", "መስከረም 18"),
    ("GZ-MEL-158", "ሐምሌ 5"),
]
...
EXPECTED_COPY_BASELINE = 3531
EXPECTED_ALWAYS = 77
EXPECTED_BOOKS = 415
EXPECTED_CALENDAR_DAYS = 366
EXPECTED_SEASONS = 4
EXPECTED_NOTES_AFTER = {"MONTHLY": 2616, "ANNUAL": 476, "ANNUAL+MONTHLY": 366, "ALWAYS": 77}

DATE_TESTS = [
    ("GZ-MEL-038", 4, 18, 1, "ANNUAL"),
    ("GZ-MEL-038", 11, 26, 1, "ANNUAL"),
    ("GZ-MEL-135", 1, 18, 1, "ANNUAL"),
    ("GZ-MEL-158", 11, 5, 1, "ANNUAL"),
]

ABSENT_TESTS = [
    ("GZ-MEL-038", 1, 26),   # no monthly day-26 cycle for this book
    ("GZ-MEL-038", 4, 17),
    ("GZ-MEL-038", 11, 25),
    ("GZ-MEL-135", 2, 18),   # only Meskerem 18
    ("GZ-MEL-135", 4, 18),   # not Tahsas 18 (that is Selama)
    ("GZ-MEL-135", 11, 21),  # birth date not used
    ("GZ-MEL-158", 10, 5),   # WRONG Sene 5 must be absent
    ("GZ-MEL-158", 11, 6),
]
...
PRESERVE_ALWAYS = ["AM-BEE-001", "AM-LIT-001", "GZ-MEL-156", "OR-WUD-001", "TI-WUD-002"]
...
def main():
    ...
    lines.append("INSERT CONFIRMED 3 BOOKS REPORT (Selama / Ewostatewos / Petros-Pawlos)")
    lines.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    lines.append("Write target: WORKING COPY only")
    lines.append("User-verified sources: Frumentius feasts, Ewostatewos tazkar, Peter&Paul Hamle 5 correction")
    ...
    if len(BOOKS) != 3:
        raise RuntimeError(f"Expected 3 books, got {len(BOOKS)}")
    ...
    lines.append("3 confirmed books inserted (Selama, Ewostatewos, Petros & Pawlos).")
    lines.append(f"{len(insert_rows)} calendar rows inserted on working copy only.")
    lines.append("Peter & Paul corrected to Hamle 5; wrong Sene 5 verified absent.")
